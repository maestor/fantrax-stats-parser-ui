...
- **Coverage thresholds**: `npm run verify` enforces minimum coverage of 94% statements, 85% branches, 95% functions, and 95% lines via `angular.json` (`architect.test.options.coverageThresholds`)
...
- When a route can auto-open accordions, panels, or expanded rows from persisted/shared settings, do **not** assume a collapsed starting state in Playwright. CI starts from the app defaults, so draft list views already open the default selected team, and leaderboard views can already move keyboard focus to that team unless the test explicitly changes that setting first
