...
function createFixture<T extends RegularLeaderboardEntry | PlayoffLeaderboardEntry>(fetchFn: () => Observable<T[]>, columns: Column[] = MOCK_COLUMNS): ComponentFixture<LeaderboardComponent> {
...
describe('LeaderboardComponent', () => {
    ...
    it('should call fetchFn on init and load regular data', () => {
        const fetchFn = vi.fn().mockReturnValue(of(MOCK_REGULAR_ENTRIES));
    ...
    it('should call fetchFn on init and load playoff data', () => {
        const fetchFn = vi.fn().mockReturnValue(of(MOCK_PLAYOFF_ENTRIES));
        const fixture = createFixture(fetchFn);
        expect(fetchFn).toHaveBeenCalledTimes(1);
        expect(fixture.componentInstance.data.length).toBe(3);
    });
    ...
    it('should set apiError and clear loading on API failure', async () => {
        ...
        expect(errorFixture.componentInstance.apiError).toBe(true);
    ...
    it('should set loading to true before data arrives, false after', async () => {
        ...
        expect(pendingFixture.componentInstance.loading).toBe(true);

        subject$.next(MOCK_REGULAR_ENTRIES);
        subject$.complete();
        pendingFixture.detectChanges();

        expect(pendingFixture.componentInstance.loading).toBe(false);
