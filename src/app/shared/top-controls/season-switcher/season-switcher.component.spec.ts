...
describe('SeasonSwitcherComponent — desktop user flow', { timeout: 60_000 }, () => {
    ...
    it('lets user select a specific season and switch back to all seasons', async () => {
        ...
        const seasonCombobox = await screen.findByRole('combobox', { name: /season\.selector/ });
    ...
    it('keeps the trigger blank until delayed season options resolve for a persisted saved season', async () => {
        ...
        const getSeasons = vi.fn((_reportType?: string, _teamId?: string, startFrom?: number) =>
            startFrom === 2012
