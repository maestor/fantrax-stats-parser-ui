...
describe('PlayerCardSeasonsService', () => {
  ...
  it('computes careerBests — max for saves, min for gaa', () => {
    ...
    expect(result.careerBests.get('saves')?.has(2024)).toBeTrue();
    expect(result.careerBests.get('saves')?.has(2023)).toBeFalse();
    expect(result.careerBests.get('gaa')?.has(2024)).toBeTrue();
  });
  ...
  describe('isCareerBest', () => {
    it('returns true when season has the best value for that column', () => {
      ...
      expect(service.isCareerBest(bests, 'saves', 2024)).toBeTrue();
      expect(service.isCareerBest(bests, 'saves', 2023)).toBeFalse();
    });

    it('returns false for unknown column', () => {
      expect(service.isCareerBest(new Map(), 'unknown', 2024)).toBeFalse();
    });
  });
});
